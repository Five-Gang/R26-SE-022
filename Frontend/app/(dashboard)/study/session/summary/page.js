...
export default function SummaryPage() {
  ...
  useEffect(() => {
    ...
    summariesApi.get(summaryId)
      .then((data) => {
        setSummary(data);
        // Animate text appearing like ChatGPT typewriter
        setVisibleChars(0);
        setStreaming(true);
      })
      .catch(console.error)
  ...
  // Typewriter effect
  useEffect(() => {
    if (!summary?.content || !streaming) return;
    const total = summary.content.length;
    if (visibleChars >= total) { setStreaming(false); return; }
    // Render fast in large chunks (50 chars/frame) for smooth feel
    const timer = setTimeout(() => {
      setVisibleChars((v) => Math.min(v + 80, total));
    }, 8);
    return () => clearTimeout(timer);
  }, [summary, visibleChars, streaming]);

  const displayedContent = summary?.content?.slice(0, visibleChars) ?? '';
